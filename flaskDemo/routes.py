...
from flaskDemo.forms import ChangePasswordForm,ForgetPasswordForm,StartForm,BuildingForm,RegistrationForm,LoginForm,MaintenanceForm,ApartmentRehabForm,LandscapingForm,PestControlForm,OtherForm
...
def register():
    ...
    if form.validate_on_submit():
        ...
        Employee = employee(firstName=form.firstName.data,lastName=form.lastName.data,username=form.username.data,password=hashed_password,phoneNumber = form.phone.data,email = form.email.data)
...
def start(worktype,buildingname):
    ...
    if form.validate_on_submit():
            Work = work(employeeID = current_user.employeeID,workType = worktype,buildingID =form.buildingName.data ,workOrdernumber=form.workOrdernumber.data,startTimeAuto=datetime.now(),endTimeAuto = None,startTimeManual = form.startTime.data, endTimeManual=None)
            db.session.add(Work)
            db.session.commit()
            flash("Start Form submitted!","success")
